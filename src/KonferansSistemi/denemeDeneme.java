...
public class denemeDeneme {
    ...
    static String[][] hakemDepo= new String[hakemSayisi()+1][14];;
    static int k=0;
    ...
    static int number=0;
    static int boy=0;
    static String[][] yazarDepo= new String[yazarSayisi()+1][15];
    static int t=0;
    
    static String[] konuID;
		...
		for(k=0;k<hakemSayisi();k++)
		{
			...
			for(int t=0;t<yazarSayisi();t++)
			{
				...
				for(int z=0;z<12;z++) 
				{
					...
					if(!hakemDepo[k][13].equals(yazarDepo[t][13]))
					{
						
						if(hakemDepo[k][z].equals(yazarDepo[t][z]))
						{
		    		    				
		    				konuID1[k][b]=yazarDepo[t][12];
		    				b++;
		    				break;
		    				}
						}
		...
		for(k=0;k<hakemSayisi();k++) {
			...
			for(int s=0;s<b;s++) {
				...
				if(konuID1[k][s]=="bos")
					...
					//konuID[t]=konuID1[k][s];
				
					System.out.println(konuID1[k][s]);
				}
